...
- All app state is one plain object (shape defined by `emptyState()` in `js/store.jsx`): `schedule` (weekly block slots per weekday), `recurring` (weekly-target task templates), `tasks`, `log`, `reviews`.
...
- Weeks always start Monday (`mondayOf`, `weekdayKey`).
